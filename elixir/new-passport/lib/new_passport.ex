defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, entrance_timestamp} <- enter_building(now),
         {:ok, counter_manual} <- find_counter_information(now),
         counter <- counter_manual.(birthday),
         {:ok, checksum} <- stamp_form(entrance_timestamp, counter, form) do
      {:ok, get_new_passport_number(entrance_timestamp, counter, checksum)}
    else
      {:error, message} -> {:error, message}
      {:coffee_break, _message} -> {:retry, NaiveDateTime.add(now, 15 * 60)}
    end
  end
